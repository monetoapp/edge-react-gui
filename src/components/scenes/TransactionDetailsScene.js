...
export class TransactionDetailsComponent extends React.Component<Props, State> {
  ...
  renderExchangeData = (crypto) => {
    ...
    const createExchangeDataString = (newline: string = '\n') => {
      ...
      }: ${123} ${crypto.symbolString}${newline}${s.strings.string_to_capitalize}: ${destinationAmount} ${destinationCurrencyCode}${newline}${
      ...
      <>
        <Tile type="touchable" title={s.strings.transaction_details_exchange_details} onPress={openExchangeDetails}>
          <View style={styles.tileColumn}>
            <FormattedText style={styles.tileTextBottom}>{`${s.strings.title_exchange} ${sourceAmount} ${crypto.symbolString}`}</FormattedText>
      ...
      <SceneWrapper background="theme">
        <ScrollView>
          <View style={styles.tilesContainer}>
            ...
            {this.renderExchangeData(crypto)}
